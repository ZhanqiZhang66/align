...
import sys
# Import torch first - lm_decoder.so depends on PyTorch symbols
import torch
sys.path.insert(0, '/victoriapvc/data/willett/lm/languageModel')
import lm_decoder
...
def unpack_batch_5or7(batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
	"""
	Unpack batch that can have either 5 or 7 items:
...
def evaluate_model(model, loadedData, partition='test', device='cuda', verbose=False):
    """Evaluate model and compute CER per day, ensuring all tensors are on the correct device."""
    ...
    if partition == "competition":
        ...
        day_indices = list(range(len(loadedData[partition])))
    print(f"Evaluating model on {partition} set with day indices: {day_indices}")

    # Accumulators
    outputs = {
        "logits": [], "logitLengths": [], "trueSeqs": [], "decodedSeqs": [], "transcriptions": [], "dayIdx": []
    }
    ...
    for day_idx in day_indices:
        ...
        loader = DataLoader(SpeechDataset([one_day]), batch_size=1, shuffle=False, num_workers=0)

        day_edit, day_len = 0, 0

        for j, batch in enumerate(loader):
            # Use unpack_batch_5or7 to handle both 5 and 7 item batches
            X, X_len, y, y_len, dayIdx, y2, y2_len = unpack_batch_5or7(batch)

            # Make sure all tensors are on the target device
            ...
            if y2_len is not None:
                y2_len = y2_len.to(device, non_blocking=True)
            # dayIdx by default is an integer or a 0-d tensor, move to device
            if torch.is_tensor(dayIdx):
                dayIdx = dayIdx.to(device, non_blocking=True)
            else:
                dayIdx = torch.tensor(dayIdx, dtype=torch.int64, device=device)

            # for passing to model, ensure day_tensor is on device
            ...
            # Sometimes model.compute_length might return cpu tensors; ensure on device
            out_lens = out_lens.to(device)

            # Batch loop
            B = pred.shape[0]
            for b in range(B):
                # Make sure index and slicing ops work on cpu
                tlen = int(y_len[b].item())
                true_seq_tensor = y[b][:tlen]
                true_seq = np.array(true_seq_tensor.detach().cpu().numpy())
                ...
                # Remove blank labels (0), still on device; move to cpu/numpy for edit distance, etc.
                decoded = decoded[decoded != 0].detach().cpu().numpy()
                outputs["decodedSeqs"].append(decoded)
                outputs["dayIdx"].append(int(day_idx))
        ...
        if day_len > 0:
            ...
            if verbose:
                print(f"PER DAY {day_idx}: {day_cer:.6f}")

    cer = (total_edit / total_len) if total_len > 0 else float("nan")
    if verbose:
        print("Model performance (PER):", cer)
