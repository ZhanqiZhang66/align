...
from neural_decoder.dataset import pad_to_multiple
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None, return_layer=None):
        """
        Args:
            x: input tensor
            mask: temporal mask
            return_layer: if specified (int), return tuple (intermediate_embedding, final_output)
                         if None, return final normalized output only
        """
        ...
        if return_layer is not None:
            # Return both intermediate (if found) and final
            if intermediate_rep is None:
                # return_layer was >= depth, use final before norm as intermediate
                intermediate_rep = x
            return intermediate_rep, x_final
        else:
            return x_final
...
class BiT_Phoneme(nn.Module):
    """
    Your original model, with minimal DANN additions:
      - forward(..., return_rep=True) returns (out, rep, None) or (out, out_2, rep, None)
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Split like your original
        self.to_patch = self.to_patch_embedding[0]
        self.patch_to_emb = nn.Sequential(*self.to_patch_embedding[1:])

        self.gaussianSmoother = GaussianSmoothing(patch_width, 20, self.gaussianSmoothWidth, dim=1)
        ...
        self.transformer = Transformer(
            dim, depth, heads, dim_head, mlp_dim_ratio, dropout, use_relative_bias=self.T5_style_pos
        )
        if bottleneck_dim is not None:
            self.bottleneck = nn.Linear(dim, bottleneck_dim)
            self.bottleneck_relu = nn.ReLU()
            self.projection = nn.Linear(bottleneck_dim, nClasses + 1)
        else:
    ...
    def forward(self, neuralInput, X_len, day_idx=None, return_rep: bool = False, rep_layer_idx=None):
        """
        Same as your original forward, plus:
          - if return_rep=True: also returns transformer reps (x) for DANN pooling
          - rep_layer_idx: if specified (int), return intermediate layer i's embedding instead of final layer
                         if None, return final layer embedding (default behavior)
        """
        ...
        if self.training and self.max_mask_pct > 0:
            ...
            if self.consistency:
                ...
                if torch.equal(mask1, mask2):
                    print("Warning: mask1 == mask2 — check randomness")
        ...
        # Determine the representation based on rep_layer_idx and return_rep

        rep_from_patch = None
        if rep_layer_idx == -1 and return_rep:
            # If rep_layer_idx == -1, use _patch_ embedding (BEFORE dropout, pos emb, transformer)
            rep_from_patch = x
        ...
        if rep_layer_idx is not None and rep_layer_idx > -1:
            # If rep_layer_idx > -1, return intermediate layer output as rep
            x_rep, x_final = self.transformer(x, mask=temporal_mask, return_layer=rep_layer_idx)
            if rep_layer_idx == self.depth:
                # this is the final transformer layer and layer-normalized! 
                x_rep = x_final
            if self.normalize_features:
                x_rep = F.normalize(x_rep, dim=-1)
                x_final = F.normalize(x_final, dim=-1)
        elif rep_layer_idx == -1 and return_rep:
            # If rep_layer_idx == -1 and return_rep, use patch embedding as rep, transformer for final
            x_final = self.transformer(x, mask=temporal_mask)
            x_rep = rep_from_patch
            if self.normalize_features:
                x_final = F.normalize(x_final, dim=-1)
                x_rep = F.normalize(x_rep, dim=-1) if x_rep is not None else None
        else:
            # Default: Just run transformer
            x_final = self.transformer(x, mask=temporal_mask)
            x_rep = x_final if return_rep else None
            if self.normalize_features:
                x_final = F.normalize(x_final, dim=-1)
                x_rep = x_final if return_rep else None
        if self.bottleneck_dim is not None:
            x_final = self.bottleneck(x_final)
            x_rep = self.bottleneck_relu(x_final)
            out = self.projection(x_rep)
        else:
            out = self.projection(x_final)

        if self.nClasses_2 is not None:
            out_2 = self.projection_2(x_final)
            if return_rep:
                # keep "(pred, rep, fut_pred)" style usable:
                return out, out_2, x_rep, None
            return out, out_2

        if return_rep:
            return out, x_rep, x_final
        return out
