...
def lm_decode(decoder, logits, returnNBest=False, rescore=False,
    ...
    if logPriors is None:
        # Match logits shape [T, C] for the native decoder binding.
        # Some builds are strict about this and may crash on shape mismatches.
        logPriors = np.zeros_like(logits, dtype=np.float32)
    else:
        logPriors = np.asarray(logPriors, dtype=np.float32)
        if logPriors.shape != logits.shape:
            if logPriors.shape == (1, logits.shape[1]):
                logPriors = np.repeat(logPriors, logits.shape[0], axis=0)
            else:
                raise ValueError(
                    f"logPriors shape {logPriors.shape} must match logits shape {logits.shape} "
                    "or be (1, C)."
                )
    logits = np.ascontiguousarray(logits, dtype=np.float32)
    logPriors = np.ascontiguousarray(logPriors, dtype=np.float32)
