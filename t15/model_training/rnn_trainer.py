...
class BrainToTextDecoder_Trainer:
    ...
    def __init__(self, args):
        ...
        # Skip torch.compile: model uses day_idx tensor indexing (day_weights[i] for i in day_idx),
        # which dynamo cannot trace (TensorVariable is not a constant). Use eager mode for training.
        if self.args.get("use_torch_compile", False):
            self.logger.info("Using torch.compile")
            self.model = torch.compile(self.model)
        else:
            self.logger.info("Using eager mode (torch.compile disabled; use use_torch_compile: true to enable)")
        ...
        prefetch_factor = self.args['dataset'].get('prefetch_factor', 2)
        persistent_workers = num_workers > 0 and self.args['dataset'].get('persistent_workers', True)
        ...
        val_workers = self.args['dataset'].get('num_val_dataloader_workers', 0)
        ...
        if self.wandb_initialized and wandb.run is not None:
            ...
            self.logger.info("✅ WandB watching model for gradients and parameters")
            ...
            if i % self.args['batches_per_val_step'] == 0 or i == ((self.args['num_training_batches'] - 1)):
                ...
                if self.wandb_initialized and wandb.run is not None:
                    log_dict = {
                        ...
                        "eval/cer": val_metrics["avg_PER"],  # PER is same as CER (Character Error Rate)
                        "eval/time": val_step_duration,
                        "batch": i
                    }
                    
                    # Log individual day PERs if enabled
                    if self.args['log_individual_day_val_PER']:
                        for day in val_metrics['day_PERs'].keys():
                            ...
                            log_dict[f"eval/cer_day_{session_name}"] = day_per
